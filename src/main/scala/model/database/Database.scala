...
class Database extends DatabaseAPI{
  ...
  def setupTable(): Unit = {
    ...
    statement.execute("CREATE TABLE IF NOT EXISTS tasks (title TEXT, description TEXT, created TEXT, due TEXT, id TEXT)")
  }
  ...
  override def addTask(task: Task): Unit = {
    ...
    statement.setString(4, task.due)
  ...
  override def getTasks: List[Task] = {
    ...
    while (result.next()) {
      ...
      val date = result.getString("created")
      val due = result.getString("due")
      val id = result.getString("id")
      tasks = new Task(title, description, date, due, id) :: tasks
    }
