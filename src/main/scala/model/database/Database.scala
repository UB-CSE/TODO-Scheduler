...
class Database extends DatabaseAPI{
  ...
  def setupTable(): Unit = {
    ...
    statement.execute("CREATE TABLE IF NOT EXISTS tasks (title TEXT, description TEXT, priority TEXT, dueTimeDate TEXT, id TEXT)")
  }
  ...
  override def addTask(task: Task): Unit = {
    ...
    statement.setString(4, task.dueTimeDate)
  ...
  override def getTasks: List[Task] = {
    ...
    while (result.next()) {
      ...
      val dueTimeDate = result.getString("dueTimeDate")
      val id = result.getString("id")
      tasks = new Task(title, description, priority, dueTimeDate, id) :: tasks
    }
