...
class Database extends DatabaseAPI{
  ...
  def setupTable(): Unit = {
    ...
    statement.execute("CREATE TABLE IF NOT EXISTS tasks (title TEXT, description TEXT, priority TEXT, id TEXT)")
  }
  ...
  override def getTasks: List[Task] = {
    val statement = connection.prepareStatement("SELECT * FROM tasks ORDER BY priority DESC")
    ...
    while (result.next()) {
      ...
      tasks = new Task(title, description, priority, id) :: tasks
    }
