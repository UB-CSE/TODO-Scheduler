...
class Database extends DatabaseAPI{
  ...
  def setupTable(): Unit = {
    ...
    statement.execute("CREATE TABLE IF NOT EXISTS tasks (title TEXT, description TEXT, due TEXT, comment TEXT, id TEXT)")
  }
  ...
  override def addTask(task: Task): Unit = {
    ...
    statement.setString(3, task.dueDate)
    statement.setString(4, task.comment)
  ...
  override def getTasks: List[Task] = {
    ...
    while (result.next()) {
      ...
      val dueDate = result.getString("due")
      val comment = result.getString("comment")
      val id = result.getString("id")
      tasks = new Task(title, description, dueDate, comment, id) :: tasks
    }
