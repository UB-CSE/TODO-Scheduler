...
class Database extends DatabaseAPI{
  ...
  def setupTable(): Unit = {
    ...
    statement.execute("CREATE TABLE IF NOT EXISTS tasks (title TEXT, description TEXT, id TEXT,priorities TEXT)")
  }
  ...
  override def addTask(task: Task): Unit = {
    ...
    statement.setString(4,task.priorities)
    statement.execute()
  }
  ...
  override def getTasks: List[Task] = {
    ...
    while (result.next()) {
      ...
      val pri = result.getString("priorities")
      tasks = new Task(title, description, id,pri) :: tasks
    }
