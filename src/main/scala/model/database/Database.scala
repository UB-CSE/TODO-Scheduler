...
class Database extends DatabaseAPI{
  ...
  def setupTable(): Unit = {
    ...
    statement.execute("CREATE TABLE IF NOT EXISTS tasks (title TEXT, description TEXT, id TEXT, time_in_hours FLOAT)")
  }
  ...
  override def getTasks: List[Task] = {
    ...
    while (result.next()) {
      ...
      val tt= result.getString("estimated time")
      tasks = new Task(title, description, id, tt) :: tasks
    }
