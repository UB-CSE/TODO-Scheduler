...
class Database extends DatabaseAPI{
  ...
  override def registerUser(username: String, password: String): Boolean = {
    ...
    val statement = connection.prepareStatement("INSERT INTO users VALUE (?, ?)")
  ...
  def getUser(uname: String): User = {
    val statement = connection.prepareStatement("SELECT * FROM users WHERE username=uname LIMIT 1")
    val result: ResultSet = statement.executeQuery()
