...
class Database extends DatabaseAPI{
  ...
  def setupTable(): Unit = {
    ...
    statement.execute("CREATE TABLE IF NOT EXISTS tasks (title TEXT, description TEXT, id TEXT, time_task TEXT,priority TEXT )")
  }
  ...
  override def getTasks: List[Task] = {
    ...
    while (result.next()) {
      ...
      val time = result.getString("time_task")
      val priority = result.getString("priority")
      tasks = new Task(title, description, id,time,priority) :: tasks
    }
