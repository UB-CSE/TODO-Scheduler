...
class Database extends DatabaseAPI{
  ...
  def setupTable(): Unit = {
    ...
    statement.execute("CREATE TABLE IF NOT EXISTS tasks (title TEXT, description TEXT, id TEXT, dateTime TEXT)")
  }
  ...
  override def addTask(task: Task): Unit = {
    ...
    statement.setString(4, task.dateTime)
  ...
  override def getTasks: List[Task] = {
    ...
    while (result.next()) {
      ...
      tasks = new Task(title, description, id, dateTime) :: tasks
    }
