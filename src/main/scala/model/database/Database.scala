...
class Database extends DatabaseAPI{
  ...
  def setupTable(): Unit = {
    ...
    statement.execute("CREATE TABLE IF NOT EXISTS tasks (title TEXT, description TEXT,ddl TEXT , id TEXT)")
  }
  ...
  override def addTask(task: Task): Unit = {
    ...
    statement.setString(3, task.ddl)
  ...
  override def getTasks: List[Task] = {
    ...
    while (result.next()) {
      ...
      val ddl = result.getString("ddl")
      val id = result.getString("id")
      tasks = new Task(title, description, ddl,  id) :: tasks
    }
