...
class Database extends DatabaseAPI{
  ...
  def setupTable(): Unit = {
    ...
    statement.execute("CREATE TABLE IF NOT EXISTS tasks (title TEXT, description TEXT, id TEXT, estimation TEXT)")
  }
  ...
  override def addTask(task: Task): Unit = {
    ...
    statement.setString(4, task.estimation)
  ...
  override def getTasks: List[Task] = {
    ...
    while (result.next()) {
      ...
      val estimation = result.getString("estimation")
      tasks = new Task(title, description, id, estimation) :: tasks
    }
