...
class Database extends DatabaseAPI{
  ...
  def setupTable(): Unit = {
    ...
    statement.execute("CREATE TABLE IF NOT EXISTS tasks (title TEXT, description TEXT, id TEXT, username TEXT)")
  }
  ...
  override def addTask(task: Task): Unit = {
    ...
    statement.setString(4,task.userN)
  ...
  override def getTasks: List[Task] = {
    ...
    while (result.next()) {
      ...
      tasks = new Task(title, description, id, username) :: tasks
    }
