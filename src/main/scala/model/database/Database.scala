...
class Database extends DatabaseAPI{
  ...
  def setupTable(): Unit = {
    ...
    statement.execute("CREATE TABLE IF NOT EXISTS tasks (title TEXT, description TEXT, date TEXT, created TEXT, id TEXT)")
  }
  ...
  override def addTask(task: Task): Unit = {
    ...
    statement.setString(4, task.created)
  ...
  override def getTasks: List[Task] = {
    ...
    while (result.next()) {
      ...
      tasks = new Task(title, description, date, created, id) :: tasks
    }
