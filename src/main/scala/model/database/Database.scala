...
class Database extends DatabaseAPI{
  ...
  def setupTable(): Unit = {
    ...
    statement.execute("CREATE TABLE IF NOT EXISTS tasks (title TEXT, description TEXT, id TEXT,dueDate TEXT)")
  }
  ...
  override def getTasks: List[Task] = {
   ...
   //   val timeCreated = result.getString("CURRENT_TIME")
      tasks = new Task(title, description, id,dueDate ) :: tasks
    }
