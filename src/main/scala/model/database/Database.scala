...
class Database extends DatabaseAPI{
  ...
  def setupTable(): Unit = {
    ...
    statement.execute("CREATE TABLE IF NOT EXISTS tasks (title TEXT, deadline TEXT, description TEXT, id TEXT)")
  }
  ...
  override def addTask(task: Task): Unit = {
    ...
    statement.setString(2, task.deadline)
  ...
  override def getTasks: List[Task] = {
    ...
    while (result.next()) {
      ...
      tasks = new Task(title, deadline, description, id) :: tasks
    }
