...
class Database extends DatabaseAPI{
  ...
  def setupTable(): Unit = {
    ...
    statement.execute("CREATE TABLE IF NOT EXISTS tasks (title TEXT, description TEXT, hoursToComplete TEXT, id TEXT)")
  }
  ...
  override def addTask(task: Task): Unit = {
    ...
    statement.setString(3, task.hoursToComplete)
  ...
  override def getTasks: List[Task] = {
    ...
    while (result.next()) {
      ...
      val hoursToComplete = result.getString("hoursToComplete")
      val id = result.getString("id")

      tasks = new Task(title, description, hoursToComplete, id) :: tasks
    }
